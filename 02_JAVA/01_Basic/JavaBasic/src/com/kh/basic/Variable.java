...
public class Variable {
	...
	public static void main(String[] args) {
		...
		/*
		 * 이스케이프 시퀀스
		 * 문자열 내애서 탭, 백슬러시, 작은 따음표등을 사용하기 위한 방식
		 */
...
//		MY_AGE = 111; --값을 한번 초기화하면 더이상 변경할 수 없음
		...
		/*
		 * 프로그래밍에서 이름 짓는 방식
		 * 카멜케이스 : 단어를 나열하여 이름을 짓되, 두번째 단어부터는 단어의 첫글자를 대문자로 시작(자바에서 모든 메서드, 변수명)
		 * ex) userName, totalPrice
		 * 파스칼케이스 : 카멜케이스에서 첫글자도 대문자로(자바에서는 클래스명에 사용)
		 * 
		 ...
		 * ex) USER_NAME
		 * 
		 * 케밥케이스 : 단어를 소문자로 작성하되 문자와 문자사이를 하이픈(-)으로 이어주는 것(html 속성, url, 설정파일등)
		 * ex) user-name
		 */
	}
