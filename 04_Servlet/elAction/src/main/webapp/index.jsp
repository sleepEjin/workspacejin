...
<body>
    ...
    <%--
        ...
        - 예 : <%@apge %>, <%@ include %>, <%@ taglib %>
        
        3. 액션태그(Action Tag)
        - JSP에서 특정 동작을 수행하도록 지시하는 태그
        - XML문법 기반으로 JSP기능을 확장
            >>표준액션태그 : <:jsp태그명></태그명> 접두어를 사용
            >>커스텀액션태그 : 별도 라이브러리를 등록하여 사용하는 태그(JSTL등...)
        --%>
		...
		3) 2개의 .jar 파일을 web inf/lib 폴더에 추가
		
		<h3>JSTL 선언방법</h3>
		<p>
			JSTL을 사용하려는 JSP 상단에 taglib지시어를 선언해야 한다. <br>
			<%-- <%@ taglib prefix="접두어" url="라이브러리파일상의 uri 주소"%> --%>
		</p>
