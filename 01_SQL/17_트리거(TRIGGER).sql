/*
    <트리거>
    특정 테이블에 대해 INSERT, UPDATE, DELETE와 같은 DML 이벤트가 발생했을 때
    자동으로 실행되는 PL/SQL코드 블록
    
    EX) 
    - 회원 탈퇴시 기존의 회원 테이블에 데이터를 DELETE하기 전에 항상 탈퇴한 회원들을 저장하는 테이블에
      INSERT 시켜줘야한다.
...
----------------------------------------------------------------
-- 입출고에 대한 데이터 기록(INSERT) 될 때마다 해당 상품에 대한 재고수량 매번 수정(UPDATE)
-- 상품테이블(재고), 입출고기록테이블()
...
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '대륙폰','샤오미',800000, DEFAULT);
