...
/*
    ...
    A(Automicity) 원자성 : 트랜잭션 내 작업은 모두 완료되거나 모두 취소되어야한다.
    C(Consistency) 일관성 : 트랜잭션 수행 전후에 DB상태가 항상 유효한 상태(제약조건 충족)야여한다.
    I(Isolation) 독립성 : 여러 트랜잭션이 동시에 실행되도 서로 간섭없이 독립적으로 실행되야한다.
    D(Duration) 지속성 : 트랜잭션이 commit되면, 그 결과는 시스템이 종료되도 영구적으로 보존되어야한다.
*/
