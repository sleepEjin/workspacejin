/*
    <함수 FUNCTION>
    전달된 칼럼값을 읽어들여서 함수(기능)를 실행한 결과를 반환
...
/*
    <문자처리함수>
    *LENGTH(칼럼 | '문자열') : 해당 문자열의 글자 수를 반환
    *LENGTHB(칼럼 | '문자열') : 해당 문자열의 바이트 수를 반환
    
     '최', '나', 'ㄱ' 한글은 글자 당 3 BYTE
    영문자, 숫자, 특수문자 글자 당 1 BYTE
...
/*
    * INSTR
    ...
    INSTR(칼럼 | '문자열', '찾고자하는 문자', ['찾을 위치의 시작값',순번]) -> 결과는 NUMBER
*/
...
--사원들 중 여자사원들만 이름, 주민등록번호를 조회
SELECT EMP_NAME, EMP_NO
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,8,1) IN('2','4');

--사원들 중 남자사원들만 이름, 주민등록번호를 조회
SELECT EMP_NAME, EMP_NO
FROM EMPLOYEE
...
/*
    ...
    [표현법]
    LTRIM/RTRIM(칼럼 | '문자열', [제거하고 싶은 문자들])
...
SELECT TRIM(LEADING'Z' FROM 'ZZZZZK HZZZ') FROM DUAL;
...
/*
    ...
    UPPER : 모든 문자열을 대문자로 변경해서 변환
    INITCAP : 띄어쓰기 기준 첫글자마다 대문자로 변경한 문자열 변환
...
-- ADD_MONTHS : 특정 날짜에 개월 수를 더한 값을 반환
...
-- 1 : 일 ~ 7: 토 
SELECT NEXT_DAY(SYSDATE,'토요일') FROM DUAL; -- 좋은 방식은 아님
...
SELECT TO_DATE('2025','YYYY'), 
        ...
        TO_DATE('60','RR') -- 두자리 입력시 현재 세기를 기준으로 자동 보정
FROM DUAL;
...
---------------------------------------------------------------------
/*
    TO_DATE : 숫자 타입 또는 문자 타입을 날짜타입으로 변경하는 함수
...
SELECT TO_DATE(051010) FROM DUAL; --0으로 시작하는 숫자는 없다 ->오류 발생
SELECT TO_DATE('051010') FROM DUAL; -- 문자열로 변경
...
--NVL2(컬럼, 반환값1, 반환값2)
-- 반환값1 : 해당칼럼이 존재하면 보여줄 값
-- 반환값2 : 해당칼럼이 존재하지 않으면 보여줄 값
...
-- 직원의 이름, 기존급여, 인상된 급여를 조회(각 직급별로 차등인상)
-- 직급이 J7인 사람은 급여를 10% 인상(SALARY*1.1)
-- 직급이 J6인 사람은 급여를 15% 인상(SALARY*1.15)
-- 직급이 J5인 사람은 급여를 20% 인상(SALARY*1.2)
