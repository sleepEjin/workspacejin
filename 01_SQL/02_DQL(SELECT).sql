 /*
    ...
    - 칼럼은 테이블에서 저장할 때 속성 = 값으로 저장
    
    => 테이블은 여러 칼럼으로 구성되고, 각 칼럼은 테이블이 표현하는 데이터의 세부적인 속성을 나타냄.
...
SELECT EMP_ID, EMP_NAME, SALARY, '원' -- 원=리터럴이라고 한다.
FROM EMPLOYEE;

/*
    <연결 연산자 : ||>
    여러 칼럼값들을 마치 하나의 칼럼처럼 연결할 수 있음
*/

SELECT EMP_NAME || '님 급여는 ' || SALARY || '원' 급여현황
FROM EMPLOYEE;

/*
    <DISTINCT>
    중복제거 - 칼럼에 표시된 값들을 한번씩만 조회하고자 할 때 사용
*/
...
/*
SELECT DISTINCT EMP NAME,DISTINCT JOB_CODE
FROM EMPLOYEE;

위처럼 사용시 에러가 발생. DISTINCT 명령어는 한번만 사용 가능
*/
...
/*
    ...
    = : 양쪽이 동일하다.
    !=, ^=, <> : 양쪽이 다르다.
    !는 같지 않다는 연산자로 제일 많이 씀
*/

--EMPLOYEE 테이블에서 부서코드가 D9인 사원들만 조회(모든 칼럼)
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
...
/*
    ...
    <BETWEEN AND>
    조건식에 사용되는 구문
    몇 이상 몇 이하인 범위에 대한 조건을 제시할 때 주로 사용되는 연산자 (이상, 이하만 가능)
    
    [표현법]
    비교 대상 칼럼 BETWEEN 하한값 AND 상한값;
*/
...
/*
    ...
    [표현법]
    비교한 대상칼럼 LIKE '특정패턴'; -> 일치하는 것만 조회
    
    특정패턴을 제시할 때 사용하는 와일드카드라는 개념이 정의되어있다.
    1. '%' : 포함문자 검색(0글자 이상 전부 조회)
    EX) 비교할 대상 칼럼 LIKE '문자%' : 비교할 대상칼럼 값 중에서 해당문자로 시작하는 값을 전부 조회
          비교할 대상 칼럼 LIKE '%문자' : 비교할 대상칼럼 값 중에서 해당문자로 끝나는 값을 전부 조회
          비교할 대상 칼럼 LIKE '%문자%' : 비교할 대상칼럼 값 중에서 해당문자로 포함된 값을 전부 조회
          
     2. '_' : 1글자를 대체검색할 때 사용
    EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할 대상컬럼 값 문자 앞에 아무글자나 딱 한글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '문자_' : 비교할 대상컬럼 값 문자 뒤에 아무글자나 딱 한글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '_문자_' : 비교할 대상컬럼 값 문자 앞뒤에 아무글자나 딱 한글자가 있는 값을 조회
...
WHERE EMAIL LIKE '__/_%' ESCAPE '/'; -- 이스케이프 문자는 어떤 것을 넣어도 상관없다.
...
/*
    ...
    SELECT문 가장 마지막 줄에 작성, 실행 순서 또한 가장 마지막에 실행이 된다.
...
--정렬 기준에 컬럼값이 동일할 경우 그 다음차순을 위해서 여러개의 컬럼을 제시할 수 있다.
...
ORDER BY 연봉 DESC
--ORDER BY 2 DESC; (왠만해서 순서를 이용해 사용하는 것은 X)
