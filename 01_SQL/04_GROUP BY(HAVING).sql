/*
    <GROUP BY>
    그룹 기준을 제시할 수 잇는 구문(해당 그룹 기준별로 여러 그룹으로 묶을 수 잇음)
...
-- GROUP BY절에 함수식 사용 가능(그룹을 나눌 수 있는 기준만 넣어주면 됨)
--남자 사원과 여자 사원의 수를 조회
SELECT DECODE(SUBSTR(EMP_NO, 8, 1),'1','남','2','여'), COUNT(*)
FROM EMPLOYEE
GROUP BY SUBSTR(EMP_NO, 8, 1);

--GROUP BY절에 여러 칼럼을 기술할 수 있음
-- 여러 개의 컬럼을 기술하면 해당 여러컬럼들 모두를 하나의 기준으로 사용함.
...
-- WHERE 절이 GROUP BY 이전에 실행되므로 해당 코드는 300만원 이상의 급여를 받는 사람을 먼저 필터링한 후
-- 부서코드와 평균을 구한다.
SELECT DEPT_CODE,ROUND(AVG(SALARY))
FROM EMPLOYEE
