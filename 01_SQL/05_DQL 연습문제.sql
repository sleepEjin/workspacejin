...
--14. EMPLOYEE 테이블에서 메일주소 '_'의 앞이 4자이면서 DEPT_CODE가 D9 또는 D6이고 고용일이 90/01/01 ~ 00/12/01이고,
...
SELECT EMP_NAME, SUBSTR(EMP_NO,1,6) AS "앞자리", 
        SUBSTR(EMP_NO,8) "뒷자리", 
        SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8,7) "합"
FROM EMPLOYEE
WHERE EMP_ID = 201;
...
--SELECT EXTRACT(YEAR FROM HIRE_DATE) "입사년도", COUNT(*) "입사 인원수"
--FROM EMPLOYEE
