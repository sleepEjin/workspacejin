...
SELECT EMP_NAME, JOB_NAME, SALARY AS "급여"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_CODE IN ('J4', 'J7') AND NVL(BONUS,0) =0;

-- 9. 부서 명과 부서 별 급여 합계 조회
SELECT DEPT_TITLE, SUM(SALARY) AS "급여 합계"
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID)
GROUP BY DEPT_TITLE;
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
HAVING SUM(SALARY) > (SELECT SUM(SALARY)*0.2 FROM EMPLOYEE); -- 서브 쿼리

-- 11. 나이 상 가장 막내의 사원 코드, 사원 명, 나이, 부서 명, 직급 명 조회  
SELECT EMP_ID, EMP_NAME, FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/365) AS "나이", 
        DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON(DEPT_CODE= DEPT_ID)
JOIN JOB USING(JOB_CODE)
...
-- 10번과 11번은 쿼리 관련으로 다음 장에 나오는 문제와 관련