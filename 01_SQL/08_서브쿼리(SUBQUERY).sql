...
-- 서브커리 예시1) 노옹철 사원과 같은 부서에 속한 사원들 전체 조회
...
/*
    *서브쿼리 구문
    ...
    -- 단일행 서브쿼리 : 서브쿼리의 조회 결과 값이 오로지 한 개일 때
...
/*
    ...
    비교대상 IN(값~) : 여러 개의 결과값 중 한 개라도 일치하는 값이 있는가?
    비교대상 >, < ANY(값~) : 여러 개의 결과값 중 한개라도 비교연산을 만족하는가?
    비교대상 >, < ALL(값~) : 여러 개의 결과값 모두가 비교연산을 만족하는가?
...
/*
    3. 다중열 서브쿼리
    결과값은 한 행이지만 나열된 컬럼수가 여러 개일 경우(두 개 이상의 컬럼)
*/
...
--  단, ROWNUM은 이미 FROM 절에서 결정이 된다.

-->인라인뷰를 통해서 TOP-N분석 : 상위 몇 개만 조회
-- 전 직원 중 급여가 가장 높은 직원 5명
SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;

-->ORDER BY절이 수행된 결과를 가지고 ROWNUM을 부여 : 상위 5명을 ROWNUM으로 조회 가능
