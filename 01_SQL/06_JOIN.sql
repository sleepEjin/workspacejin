...
FROM EMPLOYEE 
--JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE); 식별자 생성
...
/*
    ...
    단, 반드시 LEFT 또는 RIGHT를 지정해야된다.(어떤 테이블을 기준으로 할 것인가)
*/

--외부조인과 비교할만한 내부조인
-- 사원명, 부서명, 급여, 연봉
-- 부서배치를 받지 않은 2명의 사원 정보는 어떻게 되는가
...
/*
    내추럴 조인(NATURAL JOIN)
    두 테이블 간에 공통된 컬럼 이름을 기준으로 정해서 자동으로 조인하는 방식
    ON이나 USING조건을 지정하지 않아도 두 테이블간의 공통된 컬럼을 기반으로 조인
    
    동일한 컬러명이 각 테이블에 존재하는 경우에만 정상적인 조인이 가능하다.
*/
